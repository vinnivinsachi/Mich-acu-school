	...
	********/
	...
	class ProductsController extends CustomControllerAction
	{
		...
		public function indexAction()
		{
			...
			//$products = DatabaseObject_Helper_ProductDisplay::fetchVEproducts($this->db);
			//Zend_Debug::dump($products);
			$product[1]= DatabaseObject_Helper_ProductDisplay::retrieveProductFromProductsForPurchaseDetails($this->db, 1);
			
			$product[2]=  DatabaseObject_Helper_ProductDisplay::retrieveProductFromProductsForPurchaseDetails($this->db, 2);

			$product[3]=  DatabaseObject_Helper_ProductDisplay::retrieveProductFromProductsForPurchaseDetails($this->db, 3);
			$product[4]=  DatabaseObject_Helper_ProductDisplay::retrieveProductFromProductsForPurchaseDetails($this->db, 4);
			$product[5]=  DatabaseObject_Helper_ProductDisplay::retrieveProductFromProductsForPurchaseDetails($this->db, 5);
			
			foreach($product as $k=>$v){
				foreach($product[$k]["inventory"] as $inventoryK=>$inventoryK){
					$sys_bottom_size = $product[$k]["inventory"][$inventoryK]['sys_bottom_size'];
					$quantity = $product[$k]["inventory"][$inventoryK]['sys_quantity'];
					
					if($quantity>0){
					$sys_bottom_size = $this->replaceSpace($sys_bottom_size);
					$product[$k]["inventoryAvailables"][$sys_bottom_size]=$quantity; 
				//	echo 'here at setting quantity';	
					}
				}
				
				//Zend_Debug::dump($product[$k]);
			}
			...
			//Zend_Debug::dump($products);
			$this->view->secondHeader='productClassic';
