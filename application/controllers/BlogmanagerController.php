<?php
	...
	class BlogmanagerController extends CustomControllerAction
	{
		...
		public function init()
		{
			...
			$this->view->uploader_id = $this->signedInUserSessionInfoHolder->generalInfo->userID;
		}
		...
		public function indexAction()
		{
			...
			$cat = $this->getRequest()->getQuery('category');
			
			
			$totalPosts = DatabaseObject_BlogPost::GetObjectsCount($this->db, array('user_id'=>$this->signedInUserSessionInfoHolder->generalInfo->userID));
			...
			$this->view->id = $this->signedInUserSessionInfoHolder->generalInfo->userID;
			//Zend_Debug::dump($this->$identity); 
		...
		public function editAction()
		{
			...
			if($request->isPost())
			{
				...
				if($fp->process($request))
				{
					...
					DatabaseObject_Helper_ImageUpload::uploadImage($_FILES['generalImages'], $this->db,'blog_posts_images','blogposts', $fp->post->getId(),$fp->post->title);
		...
		public function previewAction()
		{
			...
			if(!DatabaseObject_StaticUtility::loadObjectForUser($post, $this->signedInUserSessionInfoHolder->generalInfo->userID, $post_id, $this->databaseColumn, $options))
			{
		...
		public function imagesAction()
		{
			if($this->request->getParam('delete'))
			{
				...
				if(DatabaseObject_Helper_ImageUpload::removeProductAndInventoryImage($this->db, 'blog_posts', $table, $tag, $image_id, $product_id, $this->signedInUserSessionInfoHolder->generalInfo->userID)){
