	...
	********/
	...
	class ProductlistingController extends CustomControllerAction
	{
		...
		public function imagesAction()
		{
			...
			elseif($this->request->getParam('reorder'))
			{
				//echo 'here';
				$order = $this->request->getParam('post_images');
				//Zend_Debug::dump($order);
				$productId = $this->request->getParam('id');
				$product=new DatabaseObject_Products($this->db);
				$product->getProductForUser($this->signedInUserSessionInfoHolder->generalInfo->userID,$productId, 'Pants');
				//echo 'here2';
